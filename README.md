...
- **动态模型目录**:分组聚合自默认分组、已有 Key 与公开 pricing 接口——未建过 Key 的分组同样可见;模型优先按 pricing 的 enable_groups 精确过滤,缺数据时回退家族启发式;图像/视频分组自动隐藏;分组每 5 分钟与网站自动同步,「立即同步」即时刷新
...
```bash
npm test   # vitest:119 个测试,覆盖脱敏、代理授权、能力解析、路由、分组目录、updater、SSE/会话解析、变更文件跟踪、文件列表/内容接口
```
