...
- **模型健康状态**:选好分组+模型后,顶栏显示近 8 小时逐 10 分钟的实测状态格(绿=正常/紫=部分失败/红=全部失败/灰=无请求)与故障率;数据来自本机凭据代理观测到的每一次真实请求,`/api/model-health/debug` 可探测网站侧状态接口
...
```bash
npm test   # vitest:130 个测试,覆盖脱敏、代理授权、能力解析、路由、分组目录、模型健康、updater、SSE/会话解析、变更文件跟踪、文件列表/内容接口
```
